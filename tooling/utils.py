from contextlib import contextmanager
from decimal import Decimal
from functools import wraps
from time import monotonic
from typing import Callable, Generator


@contextmanager
def timer(*, name: str = 'timer', display_threshold: Decimal = Decimal(0.01)) -> Generator[None, None, None]:
    before = monotonic()
    yield
    after = monotonic()

    time_elapsed = after - before
    if time_elapsed > display_threshold:
        print(f'Elapsed time of {name}: {time_elapsed}')


def timed[**P, T](callable_: Callable[P, T], /) -> Callable[P, T]:
    @wraps(wrapped=callable_)
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        callable_name = getattr(callable_, '__name__', 'anonymous_function')
        with timer(name=callable_name):
            result = callable_(*args, **kwargs)

        return result

    return wrapper
